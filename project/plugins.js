var plugins_bb40132b_638b_4a9f_b028_d3fe47acc8d1 = 
{
	...
	core.mystatusNumber = function (ctx, name, num, x, y, col2) {
		...
		if (!num && name != 'key1' && name != 'key2') {
			...
			for (var i in num) {
				core.drawIcon(ctx, 'X' + (10288), x + 5 + 10 * Number(i), y, 16, 16)
				if (num[i] === '.')
					core.drawIcon(ctx, 'X' + (10300), x + 5 + 10 * Number(i), y, 16, 16)
				else if (num[i] === '%')
					core.drawIcon(ctx, 'X' + (10301), x + 5 + 10 * Number(i), y, 16, 16)
				else if (num[i] === '-')
					core.drawIcon(ctx, 'X' + (10302), x + 5 + 10 * Number(i), y, 16, 16)
				else
					core.drawIcon(ctx, 'X' + (10289 + Number(num[i])), x + 5 + 10 * Number(i), y, 16, 16)
			}
		}
		if (name === 'key1' || name === 'key2') {
			...
			for (var k in num) {
				...
				for (var i in numk) {
					core.drawIcon(ctx, 'X' + (10288), x + 5 + 10 * Number(i) + 30 * Number(k), y, 16, 16)
					core.drawIcon(ctx, 'X' + (10289 + Number(numk[i])), x + 5 + 10 * Number(i) + 30 * Number(k), y, 16, 16)
				}
			}
			core.setFilter(ctx, '')
			name = 'key'
	...
	core.mystatusbox = function (ctx, name, num, x, y, col2) {
		...
		core.drawIcon(ctx, 'X' + (10299), x - 40 + 32, y - 10, 32, 32)
	...
	core.createSpr0 = function (des) {
		...
		core.drawImage(ctx, 'backw.png', 0, 0, 448, 448, 20, 64, 448, 448);
	...
	core.createSpr1 = function (des) {
		...
		var Scol = [250, 140, 0]
		var heroStatus2 = ['mdef', 'atk2', 'def2']
		...
		var text = [
			...
			['攻速', (hero.atk2 < 0 ? '\r[red]' : '') + hero.atk2 + '\r'],
			['防御', (hero.def < 0 ? '\r[red]' : '') + hero.def + '\r'],
			['护甲', (hero.def2 < 0 ? '\r[red]' : '') + hero.def2 + '\r'],
		...
		var text2 = [
			...
			['护盾', '每次战斗会抵挡伤害'],
			['攻击', '影响角色每次普攻伤害'],
			['攻速', '每回合普攻伤害×' + Math.max(0, (hero.atk2 * 0.01 + 1)).toFixed(2)],
			['防御', '影响角色受到攻击的直接减伤'],
